...
#define SUCC_ADRP(bp) ((char *)(bp) + WSIZE)
...
#define ROOT(index) ((char *)(heap_listp) + ((index) * WSIZE))
...
int mm_init(void)
{
    ...
    if ((heap_listp = mem_sbrk((BUCKETSIZE+4) * WSIZE)) == (void *)-1)
        return -1;
    PUT(heap_listp, 0);                                                     /* Allignment padding */
    PUT(heap_listp +(1*WSIZE), PACK((BUCKETSIZE+2)*WSIZE, 1));              /* Prologue header */
    PUT(heap_listp +((BUCKETSIZE+2)*WSIZE), PACK((BUCKETSIZE+2)*WSIZE, 1)); /* Prologue footer */
    PUT(heap_listp +((BUCKETSIZE+3)*WSIZE), PACK(0, 1));                    /* Epilogue header */
    heap_listp += DSIZE;
...
static int index_of(size_t asize) 
{
    int a[BUCKETSIZE-1]={1,2,3,4,5,6,7,8,16,32,64,128,256,512,1024,2048,4096};
    int i;
...
static void delete_block(void *bp)
{
    void *pred = GET(PRED_ADRP(bp));
    void *succ = GET(SUCC_ADRP(bp));
    if (pred != NULL) {
        PUT(SUCC_ADRP(pred), succ);
    } else {
        PUT(ROOT(index_of(GET_SIZE(HDRP(bp)))), succ);
    }
    if (succ != NULL) {
        PUT(PRED_ADRP(succ), pred);
    }
}
...
static void insert_block_at_beginning(void *bp)
{
    ...
    void *old_beginning = GET(root);
    PUT(SUCC_ADRP(bp), old_beginning);
    PUT(PRED_ADRP(bp), NULL);
    if (old_beginning != NULL) 
        PUT(PRED_ADRP(old_beginning), bp);
    PUT(root, bp);
    CHECKHEAP;
}
...
static void insert_block(void *bp)
{
    ...
    void *first_bp = GET(root);
    void *p;

    if (first_bp == NULL) {
        PUT(PRED_ADRP(bp), NULL);
        PUT(SUCC_ADRP(bp), NULL); 
        PUT(root, bp);
        CHECKHEAP;
        return;
    }

    for (p = first_bp; p != NULL; p = GET(SUCC_ADRP(p)))
    {
        if (size < GET_SIZE(HDRP(p))) {
            PUT(SUCC_ADRP(bp), p);
            PUT(PRED_ADRP(bp), GET(PRED_ADRP(p)));
            if (GET(PRED_ADRP(p)) != NULL) {
                PUT(SUCC_ADRP(GET(PRED_ADRP(p))), bp);
            } else {
                PUT(root, bp);
            }
            PUT(PRED_ADRP(p), bp);
...
static void *find_fit(size_t asize)
{
    ...
    for (int i = index_of(asize); i < BUCKETSIZE; i++) {
        for (p = GET(ROOT(i)); p != NULL; p = GET(SUCC_ADRP(p)))
        {
...
static int mm_check(void)
{
    ...
    for (int i = 0; i < BUCKETSIZE; i++) {
        root = ROOT(i);
        for (p = *root; p != NULL; p = GET(SUCC_ADRP(p)))
        {
            if (GET_ALLOC(HDRP(p)))
                return 1;
        }
